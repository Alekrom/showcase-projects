...
def get_data_rec_old(rec_train, rec_train_idx, rec_val, rec_val_idx, batch_size, num_workers, input_size,
    ...
    train_prefetch_iter = mx.io.PrefetchingIter(train_data)
    val_data = mx.io.ImageRecordIter(**log_dictionary("val", val_args))
    val_prefetch_iter = mx.io.PrefetchingIter(val_data)
    return train_prefetch_iter, val_prefetch_iter, batch_fn


def get_data_rec(data_dir, batch_size, num_workers, input_size):
    ...
    train_data = gluon.data.DataLoader(
        ...
        batch_size=batch_size, shuffle=True, last_batch='discard', num_workers=num_workers, prefetch=num_workers*4)
...
class Imagenet(Dataset):
    ...
    def get_data(self, opt):
        ...
        return get_data_rec(opt.data_dir,
                            opt.batch_size, opt.num_workers, self.get_shape(opt)[-1])
