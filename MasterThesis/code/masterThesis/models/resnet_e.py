...
from property_collector import LayerPropertyConv, LayerTypes, LayerPropertyDense, LayerPropertyAvgPooling
...
class BasicBlockE1(BasicBlockV1):
    ...
    def __init__(self, *args, use_fp=False, use_pooling=False, write_on=0, slices=1, num_groups=1, init=True, collector=None, **kwargs):
    ...
    def _init(self, collector):
        layer = nn.activated_conv(self.slice_width, kernel_size=3, stride=self.stride,
                                        padding=1, in_channels=self.in_channels)
        self.body.add(layer)
        add_properties_if_collector(
            LayerPropertyConv(channels=self.slice_width, kernel_size=3, strides=self.stride, padding=1, in_channels=self.in_channels,
                              params=layer.params, layer_type=LayerTypes.activatedConv, qconv_params=layer.qconv.params),
            collector, self.name)
        ...
        if self.downsample is not None:
            ...
            if self.use_pooling:
                conv_stride = 1
                layer = nn.AvgPool2D(pool_size=2, strides=2, padding=0)
                self.downsample.add(layer)
                add_properties_if_collector(
                    LayerPropertyAvgPooling(None, LayerTypes.avgPool, pool_size=2, strides=2, padding=0),
                collector, self.name, True)
            if self.use_fp:
                layer = nn.Conv2D(self.channels, kernel_size=1, strides=conv_stride, use_bias=False,
                                              groups=self.num_groups, in_channels=self.in_channels, prefix="sc_conv_")
                self.downsample.add(layer)
                add_properties_if_collector(
                    LayerPropertyConv(channels=self.channels, kernel_size=1, strides=conv_stride, use_bias=False, groups=self.num_groups, in_channels=self.in_channels,
                                      params=layer.params, layer_type=LayerTypes.Conv2d),
                    collector, self.name, True)
            else:
                layer = nn.activated_conv(self.channels, kernel_size=1, stride=conv_stride, padding=0,
                                                      in_channels=self.in_channels, prefix="sc_qconv_")
                self.downsample.add(layer)
                add_properties_if_collector(
                    LayerPropertyConv(channels=self.channels, kernel_size=1, strides=conv_stride, padding=0, in_channels=self.in_channels,
                                      params=layer.params, layer_type=LayerTypes.activatedConv, qconv_params=layer.qconv.params),
                    collector, self.name, True)
    ...
    def _make_layer(self, block, layers, channels, stride, stage_index, in_channels=0, collector=None, **kwargs):
        ...
        with layer.name_scope():
            layer.add(block(channels, stride, channels != in_channels, in_channels=in_channels, prefix='',
                            slices=slices, write_on=0, collector=collector, **kwargs))
            for i in range(layers_without_stride):
                write_on = (offset + i) % self.slices
                layer.add(block(channels, 1, False, in_channels=channels, prefix='',
                                slices=self.slices, write_on=write_on, collector=collector, **kwargs))
        return layer
    ...
    def __init__(self, block, layers, channels, classes=1000, initial_layers="imagenet", **kwargs):
        ...
        with self.name_scope():
            ...
            for i, num_layer in enumerate(layers):
                ...
                self.features.add(
                    self._make_layer(block, num_layer, channels[i + 1], stride, i, in_channels=channels[i], collector=collector))
...
class ResNetE1Shared(ResNetE):
    ...
    def __init__(self, channels, bits_w, bits_a, classes, **kwargs):
        self.bits_w = bits_w
        self.bits_a = bits_a
        super(ResNetE1Shared, self).__init__(channels, classes, **kwargs)
...
def get_resnet_e(version, num_layers, pretrained=False, ctx=cpu(),
    ...
    assert num_layers in resnet_spec, \
        "Invalid number of layers: %d. Options are %s"%(initial
